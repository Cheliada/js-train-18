...
function getBookById(books, id) {
  try {
    if (books.find((element) => element.id == id) !== undefined) {
...
function decodeURIComponentWrapper(encodedString) {
  try {
    const result = decodeURI(encodedString);
    return result;
} catch (error) {
...
function validateUser(user) {
  try {
    ...
    if (!user.hasOwnProperty("name")) {
        error = new Error("Ім'я користувача не вказано!", { cause: user });
        throw error;
    }
    if (!user.hasOwnProperty("email")) {
        error = new Error("Email користувача не вказано!", { cause: user });
        throw error;
    }
...
function calculateSquareRoot(number) {
  try {
    ...
    if (typeof number !== "number") {
        error = new Error("Аргумент має бути числом!");
        throw error;
    }
    if (number < 0) {
        error = new Error("Число не повинно бути від'ємним!");
        throw error;
    }
